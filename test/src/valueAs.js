...
describe('ObjectAssigner#valueAs', () => {
  it('must be work called with ("field", "fieldAs")', () => {
    ...
    let targetDescriptor = Object.getOwnPropertyDescriptor(target, 'fieldAs');
  ...
  it('must be work called with ("field", "fieldAs", #descriptor)', () => {
    let target = helper.createTarget();
    let source = helper.createSource();
    let object = new ObjectAssigner(target, source);
    let descriptor = {value: false, configurable: false, enumerable: false, writable: false};

    assert.ok(object.valueAs('field', 'fieldAs', descriptor) instanceof ObjectAssigner);
  ...
  it('must be work called with ("method", "methodAs", #descriptor)', () => {
    ...
    assert.ok(object.valueAs('method', 'methodAs', descriptor) instanceof ObjectAssigner);
